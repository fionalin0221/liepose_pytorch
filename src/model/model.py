...
class Backbone(nn.Module):
    ...
    Args:
        ...
        out_dim (int): output dimension of backbone e.g. 512
        resnet_depth (int): e.g. 34, 50
    """
    def __init__(self, image_shape, feat_dim, out_dim, resnet_depth):
...
class PosEmbed(nn.Module):
    ...
    """
    def __init__(self, in_feat_dim, embed_dim, out_dim, shift=True):
        ...
        self.mlp = nn.Linear(self.in_feat_dim * self.embed_dim, self.out_dim, bias=shift)

    def forward(self, x):
        device = x.device
        x = x.unsqueeze(-1)                                       # x:(N, feat_dim) -> (N, feat_dim, 1)
        emb = x * self.log_scale_seq.to(device)                   # x:(N, feat_dim, 1) -> emb:(N, feat_dim, half_dim)
        emb = torch.cat([torch.sin(emb), torch.cos(emb)], dim=-1) # emb:(N, feat_dim, half_dim) -> emb:(N, feat_dim, embed_dim)
        emb = emb.view((emb.shape[0], -1))                        # emb:(N, feat_dim, embed_dim) -> emb:(N, feat_dim*embed_dim)
        emb = self.mlp(emb)                                       # emb:(N, feat_dim*embed_dim) -> emb:(N, out_dim)
        return emb
...
class Head(nn.Module):
    ...
    """
    def __init__(self, in_dim, out_dim, n_layers = 1, block_type = 'MlpBlock', activ = 'leaky_relu'):
        """
        Args:
        in_dim (int): input of the head
        out_dim (int): dimension of output of head
    ...
    def broadcast_batch(self, x, bs):
        # calculate number of slices of input data
        batch_size = x.shape[0]
        slices = bs // batch_size

        # duplicate data in first dimension, repeat "slices" time
        x = x.repeat(slices, 1)
        return x
...
class Model(nn.Module):
    def __init__(self, 
        ...
        self.backbone = Backbone(image_shape, feat_dim=128, out_dim=512, resnet_depth=resnet_depth)
        self.head = Head(in_dim=in_dim, out_dim=out_dim, n_layers=mlp_layers, block_type=fourier_block, activ=activ_fn)
...
def main():
    ...
    # backbone = Backbone(image_shape=image_shape, feat_dim=64, out_dim=512, resnet_depth=34)
    # output = backbone(img)
    # print(output.shape)
